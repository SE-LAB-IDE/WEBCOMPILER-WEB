...
<body>
   ...
   <div id="mySidenav" class="sidenav">
      ...
      <div id="list1" class="list">
         ...
         <form action="algorithmList.jsp" target="_self"
            id='list<%=algorithmList1.get(i).getNum()%>' method='post'>
      ...
      <div id="list2" class="list">
         ...
         %>
         <form action="algorithmList.jsp" target="_self"
            id='list<%=algorithmList2.get(i).getNum()%>' method='post'>
            <input type="hidden" name="num"
               value='<%=algorithmList2.get(i).getNum()%>'> <a href="#"
               onclick="document.getElementById('list<%=algorithmList2.get(i).getNum()%>').submit();"><%=algorithmList2.get(i).getName()%></a>
            <%
               if (tryCnt == 0)
               out.println("<span style='color:red;'>X</span>");
            else if (tryCnt == 1)
      ...
      <div id="list3" class="list">
         ...
         %>
         <form action="algorithmList.jsp" target="_self"
            id='list<%=algorithmList3.get(i).getNum()%>' method='post'>
            <input type="hidden" name="num"
               value='<%=algorithmList3.get(i).getNum()%>'> <a href="#"
               onclick="document.getElementById('list<%=algorithmList3.get(i).getNum()%>').submit();"><%=algorithmList3.get(i).getName()%></a>
            <%
               if (tryCnt == 0)
               out.println("<span style='color:red;'>X</span>");
            else if (tryCnt == 1)
      ...
      <div id="listD" class="list">
         ...
         %>
         <form action="algorithmList.jsp" target="_self"
            id='list<%=algorithmList4.get(i).getNum()%>' method='post'>
            <input type="hidden" name="num"
               value='<%=algorithmList4.get(i).getNum()%>'> <a href="#"
               onclick="document.getElementById('list<%=algorithmList4.get(i).getNum()%>').submit();"><%=algorithmList4.get(i).getName()%></a>
            <%
               if (tryCnt == 0)
               out.println("<span style='color:red;'>X</span>");
            else if (tryCnt == 1)
   ...
   if (request.getParameter("num") != null) {//코드 설명 input ouput 출력
      ...
      if (request.getAttribute("algoResult") != null) {
         int result = Integer.parseInt((String)request.getAttribute("algoResult"));
   ...
   <div id="sourceList">
      ...
      <div>
         ...
         %>
         <a href="#"
            onclick="showOrHide('c<%=userCodeList.get(i).getCodeNum()%>')">소스보기</a>

         <form action="/algorithm/editorForAlgorithm.jsp" target="editor" method="post"
            class="toEditor">
...
window.onload=function(){ // 문제 클릭시 ide에 자동으로 문제번호 넘겨줌
   toIDE('<%=algorithmNum%>',0);
      }
</script>
<%}else{ %>
<script>
window.onload=function(){ // 문제 클릭시 ide에 자동으로 문제번호 넘겨줌
   toIDE('<%=algorithmNum%>',1);
      }
</script>
<%} %>
</body>
