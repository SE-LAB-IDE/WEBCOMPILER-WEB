...
.left_bx {
	...
	width: 201px;
	height: 900px;
	background: right top repeat-y #f9fafa;
	z-index: 2;
	margin-top: 40px;
}
...
.left_bx .qeustion {
	...
	height: 43px;
	padding-left: 22px;
	background: center no-repeat #4e545d;
	color: #fff;
	font-size: 14px;
	line-height: 43px
}
...
<script>
	...
	function inquiry() {
		window.name = "inquiry"
		window.open("../inquiry/inquriywriteform.jsp", "inquirywrite",
...
<body>
	...
	<div class="left_bx">
		...
		<ul class="my_menu">
			...
			<li><a href="" onclick="inquiry()">문의하기</a></li>
			<%
				if ("master".equals((String) session.getAttribute("authority"))) {
			%>
			<li><a href="inquiryMasterMode.jsp">문의 답변</a></li>
			<%
				}
			%>
		</ul>
	...
	<div class="bodymargin">
		...
		<div class="inquiryBox"
			onclick="showOrHide('<%=inquiryList.get(i).getNum()%>')">
			<hr />
			<table class="inquirytable">
				<tr class="inquiry_head">
					<th>문의 내역 : <%=inquiryList.get(i).getTitle()%> <%
			...
			<div id='<%=inquiryList.get(i).getNum()%>'>
				<table class="inquirytable">
					<tr class="inquiry_body">
						<td>
							&nbsp; <%=inquiryList.get(i).getText()%> <!-- 문의내용 출력 --></td>
					</tr>
				</table>
				...
				<table class="inquirytable">
					<tr class="inquiry_head">
						<th><%=vo.getTitle()%> || id: <%=vo.getId()%> <!-- 답변제목과 답변자의 id출력 -->
						</th>
					</tr>
				</table>
				<hr>
				<table class="inquirytable">
					<tr class="inquiry_body">
						<td><img src="../images/arrow.svg" class="img_icon" /> &nbsp;
							<%=vo.getText()%><!-- 답변내용 출력 --></td>
					</tr>
				</table>
				<p></p>
				<%
